 ...
 */
// ─── Rotating Text ─────────────────────────────────────────────────────────
function initRotatingText(config) {
...
function initScrollReveal(selector, config = {}) {
    const { threshold = 0.1, rootMargin = '0px 0px -50px 0px', animationClass = 'animate-reveal', } = config;
...
// ─── Contact Form ──────────────────────────────────────────────────────────
function getFormElement(formId, name) {
...
function initDemoForm(formId, formspreeId) {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        const plan = getFormElement(formId, 'plan')?.value;
        const app = getFormElement(formId, 'app')?.value;
        if (!name || !email) {
            const statusEl = form.querySelector('.form-status');
            if (statusEl) {
    ...
    openTriggers.forEach((el) => {
        el.addEventListener('click', () => {
            ...
            if (plan && targetId === 'trial-modal') {
                const planSelect = document.querySelector('#trial-plan');
                if (planSelect)
